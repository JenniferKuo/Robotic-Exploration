...
class Particle:
    ...
    def observation_model(self, lx, ly):
        ...
        # 給定landmark座標lx,ly， 送到obervasion model得到觀測結果r跟theta
        q = (lx-x)**2+(ly-y)**2
        z_r = np.sqrt(q)
        # delta = np.array([lx - x, ly - y])
        # z_r = np.sqrt((np.transpose(delta)/delta.T).dot(delta))
        z_th = np.arctan2(lx-x, ly-y) - yaw
    ...
    def compute_H(self, lx, ly):
        ...
        delta_x = lx - x
        delta_y = ly - y
        H = np.array([[delta_x/np.sqrt(q), delta_y/np.sqrt(q)], [-delta_y/q, delta_x/q]])
        return H
    ...
    def update_landmark(self, z, lid):
        ...
        if lid not in self.landmarks:
            ...
            sig = Hinv.dot(self.Qt).dot(np.transpose(Hinv) / Hinv.T)
            ...
            H = self.compute_H(mu[0,0], mu[1,0])
            Q = H.dot(sig).dot(np.transpose(H)/H.T) + self.Qt
            K = sig.dot(np.transpose(H)/H.T).dot(np.linalg.inv(Q))
            e = np.array(z) - np.array(z_hat)
            e[1] = normalize_angle(e[1])
            e = e.reshape(2,1)
            print("mu:",mu)
            self.landmarks[lid]["mu"] = mu + K.dot(e)
            self.landmarks[lid]["sig"] = (np.eye(2) - K.dot(H)).dot(sig)
...
def main():
    ...
    Qt_sim = np.diag([4, np.deg2rad(4)]) ** 2    # Observation Noise，觀測有4個pixel的誤差
