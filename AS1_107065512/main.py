...
def main():
    ...
    while(True):
        ...
        if nav_pos is not None:
            if old_nav_pos != nav_pos:
                # 0: Astar / 1: RRT Star
                if plan_type == 0:
                    path = planner.planning(start=tuple(map(int, pos[0:2])), goal=nav_pos, img=img, inter=20)
                    path = np.array(cubic_spline_2d(path, interval=1))
                elif plan_type == 1:   
                    path = planner.planning(tuple(map(int, pos[0:2])), nav_pos, 30, img)
